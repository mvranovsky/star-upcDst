...
file_path = "/gpfs01/star/pwg/mvranovsk/Run17_P20ic/AnaGoodRun_2.5.25/merged/StRP_production_0000.root" 
...
total_int_lum_corrected_events = 0.0
...
if __name__ == "__main__":
    ...
    for line in lines:
        ...
        #total_integrated_luminosity += luminosity
        total_int_lum_corrected_events += (1.0 * inst_lumi )* (nEventsJPsi / events_all)
        total_cor_int_lum_perrun += inst_lumi*(1.0*nEventsJPsi/events)*veto_correction(inst_lumi)
        total_cor_int_lum_perrun_min += inst_lumi*(veto_correction(inst_lumi)-0.01)
        total_cor_int_lum_perrun_max += inst_lumi*(veto_correction(inst_lumi)+0.01)
        total_uncorrected_lumi += inst_lumi


    CS_events, CS, CS_min, CS_max = 0, 0, 0, 0 
    total_cor_int_lum_perrun = total_cor_int_lum_perrun
    total_cor_int_lum_perrun_min = total_cor_int_lum_perrun_min/1000
    total_cor_int_lum_perrun_max = total_cor_int_lum_perrun_max/1000 
    total_int_lum_corrected_events = total_int_lum_corrected_events/1000 
    total_uncorrected_lumi = total_uncorrected_lumi/1000
    #error = 0.0
    eff_combined = 0
    if "noRP" in input_list:
        ...
        CS_events = calculateCS(total_int_lum_corrected_events, yield_withoutRP, False)
        ...
        CS_events = calculateCS(total_int_lum_corrected_events, yield_withRP, True)
    ...
    print "-------------------------------------------------------"
    print "Total integrated luminosity corrected with number of events: ", total_int_lum_corrected_events, " pb^-1"
    print "Final cross section with correction based on division of number of events:", CS_events, " pb"
    print "-------------------------------------------------------"
