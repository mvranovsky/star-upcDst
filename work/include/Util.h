...
namespace UTIL {
   ...
   enum JPSISELECTION_CUT {JPSIALL = 1, JPSITRIG, JPSI1VTX, JPSIVTXZ,JPSIBEMC , JPSIBACKTOBACK,JPSIPID,JPSIQTOT, JPSI1RP, JPSIRPFIDCUT,nJPSISelectionCuts };
...
class UTIL::Util{
   public:
      ...
      inline TString analysisJPSI(UInt_t id) const { if(id<nJPSISelectionCuts) return mJPSICutName[id]; else{ std::cerr << "ERROR in Util::analysisCutName(UInt_t id): id out of range" << std::endl; return TString("");} }
      inline TString analysisJPSI2(UInt_t id) const { if(id<nJPSI2SelectionCuts) return mJPSI2CutName[id]; else{ std::cerr << "ERROR in Util::analysisCutName(UInt_t id): id out of range" << std::endl; return TString("");} }
